...
class NvimYcm(object):
    ...
    def on_vim_enter(self, ycm_dir):
        ycmd_path = os.path.join(ycm_dir, 'third_party', 'ycmd', 'ycmd')
    ...
    def on_buffer_visit(self, data):
        ...
        add_ultisnips_data(request_data)
    ...
    def on_end_compilation(self, args):
        self.vim.eval('nvim_ycm#EndCompilation({0})'.format(dumps(args)[1:-1]))
    ...
    def on_end_completion(self, args):
        self.vim.eval('nvim_ycm#EndCompletion({0})'.format(dumps(args)[1:-1]))
...
def add_ultisnips_data(extra_data):
